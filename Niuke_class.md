...
```
     * 原理： 实质就是对每个数找右边有多少个数比它大<br>
     * 设小和为res，对于左边数组的每一个数i，归并的时候看右边数组有多少比它大(不能再看左边本身比它大的了，因为之前已经归并过计算过了)，
       然后res+i*个数,直到最后全部归并完成，把下面的红色算式全部加起来就是小和<br>
...
```
   * 逆序对问题
        ...
        ```
     * 原理：实质就是对每个数找右边有多少个数比它小<br>，(可能maybe,没有验证过）是在小和那里改为：<br>
