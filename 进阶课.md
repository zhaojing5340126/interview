...
* 问题定义：给你一个长度为N的链表。N很大，但你不知道N有多大。你的任务是从这N个元素中随机取出k个元素。你只能遍历这个链表一次。你的算法必须保证取出的元素恰好有k个，且它们是完全随机的（出现概率均等）。
...
* 【步骤】：
    * 1） 1~10 号球，吐出时直接进袋子
    * 2）吐 n 时：以 10/n 的概率决定要不要让它进袋子 【10/n的概率 用一个函数来决定：等概率返回1~n中的一个数，如果<=10，则进袋子，否则扔到】
        * 2.1)  如果进的话，要从袋子等概率扔一个
        * 2.2） 如果不进的话，则彻底扔掉这个球
    ![](./picture/蓄水池问题.png)
* 上述证明，用此方法能保证每个球最后留下来的概率都是 k/n
...
public class ReservoirSampling {
    public static int rand(int max){
        return (int) (Math.random()*max)+1; //从1~n中选出一个数
    }
    public static int[] getKNumsRand(int k,int max){ //选出 k 个数
        if(max < 1|| k<1){
            return null;
        }
        ...
        for (int i = k+1; i<max+1; i++){
            if(rand(i) <= k){ //从 1~n 中选出一个数 ，判断是否小于等于 k
                res[rand(k) -1] = i ;//从袋子扔出一个球，然后放入 k 这个球
            }
        }
        return res;
    }
